...
class P1Data(PhaseData):
    ...
    def add_shot(
        self, timestamp: datetime, size: float, vwc_before: float, vwc_after: float
    ):
...
class ZoneStateMachine:
    ...
    def __init__(
        self,
        zone_id: int,
        initial_phase: IrrigationPhase = IrrigationPhase.P2_MAINTENANCE,
        ...
        self._transition_callbacks: Dict[
            Tuple[IrrigationPhase, IrrigationPhase], List[Callable]
        ] = {}
    ...
    def can_transition(
        self, transition: PhaseTransition, to_phase: Optional[IrrigationPhase] = None
    ) -> bool:
        ...
        with self.lock:
            ...
            if to_phase:
                return any(
                    t[0] == transition and t[1] == to_phase for t in valid_transitions
                )
    ...
    def transition(
        ...
        to_phase: Optional[IrrigationPhase] = None,
        reason: str = "",
        **kwargs,
    ) -> bool:
        ...
        with self.lock:
            ...
            if to_phase is None:
                ...
                to_phase = next(
                    (t[1] for t in valid_transitions if t[0] == transition), None
                )
            ...
            self._execute_callbacks(
                self._on_exit_callbacks.get(old_phase, []), old_phase=old_phase
            )
            ...
            self._execute_callbacks(
                self._on_enter_callbacks.get(to_phase, []), new_phase=to_phase
            )
    ...
    def register_transition(
        self, from_phase: IrrigationPhase, to_phase: IrrigationPhase, callback: Callable
    ):
    ...
    def update_p0_dryback(self, current_vwc: float, peak_vwc: float):
        ...
        with self.lock:
            if (
                self.state.current_phase == IrrigationPhase.P0_MORNING_DRYBACK
                and self.state.p0_data
            ):
                self.state.p0_data.peak_vwc = peak_vwc
                self.state.p0_data.current_dryback_percentage = (
                    ((peak_vwc - current_vwc) / peak_vwc) * 100 if peak_vwc > 0 else 0
                )

                # Calculate dryback rate
                duration_minutes = self.get_phase_duration().total_seconds() / 60
                if duration_minutes > 0:
                    self.state.p0_data.dryback_rate = (
                        self.state.p0_data.current_dryback_percentage / duration_minutes
                    )
    ...
    def update_p1_progress(self, current_vwc: float):
        ...
        with self.lock:
            if (
                self.state.current_phase == IrrigationPhase.P1_RAMP_UP
                and self.state.p1_data
            ):
    ...
    def record_p2_irrigation(self):
        ...
        with self.lock:
            if (
                self.state.current_phase == IrrigationPhase.P2_MAINTENANCE
                and self.state.p2_data
            ):
    ...
    def record_p3_emergency(self):
        ...
        with self.lock:
            if (
                self.state.current_phase == IrrigationPhase.P3_PRE_LIGHTS_OFF
                and self.state.p3_data
            ):
    ...
    def get_state_summary(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                "phase_duration_minutes": self.get_phase_duration().total_seconds()
                / 60,
                ...
                "phase_history": [
                    (p.value, t.isoformat()) for p, t in self.state.phase_history[-10:]
                ],
            }
