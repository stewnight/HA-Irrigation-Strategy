...
class MasterCropSteeringApp(BaseAsyncApp):
    ...
    def zone_phases(self) -> Dict[int, str]:
        ...
        return {
            zone_id: machine.get_phase_string()
    ...
    def zone_phase_data(self) -> Dict[int, Dict]:
        ...
        for zone_id, machine in self.zone_state_machines.items():
            ...
            if (
                state.current_phase == IrrigationPhase.P0_MORNING_DRYBACK
                and state.p0_data
            ):
                ...
                legacy_data["p1_shot_history"] = [
                    (s["timestamp"], s["size"], s["vwc_before"], s["vwc_after"])
                    for s in state.p1_data.shot_history
                ]
                if state.p1_data.shot_history:
                    legacy_data["p1_last_shot_time"] = state.p1_data.shot_history[-1][
                        "timestamp"
                    ]
            elif (
                state.current_phase == IrrigationPhase.P2_MAINTENANCE and state.p2_data
            ):
    ...
    def initialize(self):
        ...
        if not self._validate_required_entities():
            self.log(
                "⚠️ WARNING: Some required entities are missing. The app will continue but some features may not work.",
                level="WARNING",
            )
        ...
        self.log(
            "📊 Modules: Dryback Detection ✓, Sensor Fusion ✓, ML Prediction ✓, Crop Profiles ✓, Dashboard ✓"
        )
        ...
        try:
            ...
            if self.args and any(key in self.args for key in ["hardware", "sensors"]):
                config = {
                    ...
                    "notification_service": self.args.get(
    ...
    def _register_phase_callbacks(self, zone_num: int, state_machine: ZoneStateMachine):
        ...
        state_machine.register_on_enter(
            IrrigationPhase.P1_RAMP_UP, lambda **kwargs: self._on_enter_p1(zone_num)
        )

        # P2 Entry - Start maintenance
        state_machine.register_on_enter(
            IrrigationPhase.P2_MAINTENANCE, lambda **kwargs: self._on_enter_p2(zone_num)
        )
        ...
        state_machine.register_on_exit(
            IrrigationPhase.P1_RAMP_UP, lambda **kwargs: self._on_exit_p1(zone_num)
        )
    ...
    def _on_exit_p1(self, zone_num: int):
        ...
        if machine.state.p1_data:
            self.log(
                f"Zone {zone_num}: P1 Summary - {machine.state.p1_data.shot_count} shots administered"
            )
    ...
    def _setup_listeners(self):
        ...
        self.listen_state(
            self._on_phase_change, "select.crop_steering_irrigation_phase"
        )
        self.listen_state(
            self._on_crop_profile_change, "select.crop_steering_crop_type"
        )
        ...
        self.listen_event(
            self._on_manual_irrigation_shot, "crop_steering_irrigation_shot"
        )

        # Listen to manual override events
        self.listen_event(
            self._on_manual_override_event, "crop_steering_manual_override"
        )

        # Listen to phase transition service calls from HA integration
        self.listen_event(
            self._on_phase_transition_service, "crop_steering_phase_transition"
        )

        # Listen to manual override service calls from HA integration
        self.listen_event(
            self._on_set_manual_override_service, "crop_steering_set_manual_override"
        )
    ...
    def _setup_timers(self):
        ...
        self.run_every(
            self._update_performance_analytics, "now+20", 300  # Every 5 minutes
        )
    ...
    def _initialize_default_crop_profile(self):
        ...
        try:
            ...
            current_crop = self.get_entity_value(
                "select.crop_steering_crop_type", default="Cannabis_Athena"
            )
            current_stage = self.get_entity_value(
                "select.crop_steering_growth_stage", default="vegetative"
            )

            profile_result = self.crop_profiles.select_profile(
                current_crop, current_stage
            )

            if profile_result["status"] == "success":
                self.log(
                    f"✅ Initialized crop profile: {current_crop} ({current_stage})"
                )
            else:
                self.log(
                    f"⚠️ Failed to initialize crop profile: {profile_result.get('message', 'Unknown error')}"
                )
    ...
    def _get_zone_schedule(self, zone_num: int) -> Dict[str, time]:
        ...
        try:
            ...
            on_hour = int(
                self._get_number_entity_value("number.crop_steering_lights_on_hour", 12)
            )
            off_hour = int(
                self._get_number_entity_value("number.crop_steering_lights_off_hour", 0)
            )
                ...
                for sensor_type in [
                    ...
                    if zone_num <= len(sensor_list):
                        ...
                        if sensor_id and not self.entity_exists(sensor_id):
                            warnings.append(
                                f"Zone {zone_num} sensor {sensor_type}: {sensor_id}"
                            )
    ...
    async def _create_initial_sensors(self, kwargs):
        ...
        try:
            ...
            phase_summary = ", ".join(
                [f"Z{z}:{p}" for z, p in self.zone_phases.items()]
            )
            await self.async_set_entity_value(
                ...
                attributes={
                    ...
                    "zone_phases": {
                        str(k): str(v) for k, v in self.zone_phases.items()
                    },
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                await self.async_set_entity_value(
                    f"sensor.crop_steering_zone_{zone_num}_phase",
                    phase,
                    attributes={
    ...
    async def _update_zone_water_usage(self, zone_num: int, duration_seconds: float):
        ...
        try:
            ...
            dripper_flow_rate = self._get_number_entity_value(
                "number.crop_steering_dripper_flow_rate", 1.2
            )  # L/hour per dripper
            drippers_per_plant = self._get_number_entity_value(
                "number.crop_steering_drippers_per_plant", 2
            )
            num_plants = self._get_number_entity_value(
                f"number.crop_steering_zone_{zone_num}_plant_count", 4
            )
            shot_multiplier = self._get_number_entity_value(
                f"number.crop_steering_zone_{zone_num}_shot_size_multiplier", 1.0
            )
            ...
            # Check daily limit
            max_daily = self._get_number_entity_value(
                f"number.crop_steering_zone_{zone_num}_max_daily_volume", 20.0
            )
            if zone_data["daily_total"] >= max_daily:
                self.log(
                    f"⚠️ Zone {zone_num} daily water limit reached: {zone_data['daily_total']:.1f}L >= {max_daily}L",
                    level="WARNING",
                )

        except Exception as e:
            self.log(
                f"❌ Error updating zone {zone_num} water usage: {e}", level="ERROR"
            )
        ...
        try:
            ...
            await self.async_set_entity_value(
                ...
                attributes={
                    ...
                    "last_reset": str(
                        zone_data.get("last_reset_daily", datetime.now().date())
                    ),
                },
            )
            ...
            await self.async_set_entity_value(
                ...
                attributes={
                    ...
                    "state_class": "total_increasing",
                    "last_reset": str(
                        zone_data.get("last_reset_weekly", datetime.now().date())
                    ),
                },
            )
            ...
            last_irrigation = self.zone_phase_data.get(zone_num, {}).get(
            ...
            self.log(
                f"❌ Error updating zone {zone_num} water sensors: {e}", level="ERROR"
            )
    ...
    def _get_state_file_path(self) -> str:
        ...
        try:
            ...
            if not os.path.exists(config_dir):
                ...
                config_dir = "/tmp"
                self.log(
                    f"⚠️ Config directory not found, using temp: {config_dir}",
                    level="WARNING",
                )
            ...
            try:
                ...
                self.log(
                    f"⚠️ No write permission to {config_dir}, using /tmp",
                    level="WARNING",
                )
    ...
    def _save_persistent_state(self, kwargs=None):
        ...
        try:
            state_data = {
                ...
                "zone_water_usage": {},
                "last_irrigation_time": (
                    self.last_irrigation_time.isoformat()
            ...
            for zone_num, data in self.zone_phase_data.items():
                state_data["zone_phase_data"][zone_num] = {
                    "p0_start_time": (
                        data["p0_start_time"].isoformat()
                        if data["p0_start_time"]
                        else None
                    ),
                    "p0_peak_vwc": data["p0_peak_vwc"],
                    "last_irrigation_time": (
                        data["last_irrigation_time"].isoformat()
                        if data["last_irrigation_time"]
                        else None
                    ),
                }
            ...
            for zone_num, data in self.zone_water_usage.items():
                state_data["zone_water_usage"][zone_num] = {
                    ...
                    "last_reset_daily": (
                        data["last_reset_daily"].isoformat()
                        if data["last_reset_daily"]
                        else None
                    ),
                    "last_reset_weekly": (
                        data["last_reset_weekly"].isoformat()
                        if data["last_reset_weekly"]
                        else None
                    ),
                }
    ...
    def _load_persistent_state(self):
        ...
        try:
            ...
            if file_size > 10 * 1024 * 1024:  # 10MB limit
                self.log(
                    f"⚠️ State file too large ({file_size} bytes), skipping load",
                    level="WARNING",
                )
                return
            ...
            try:
                ...
                backup_file = (
                    state_file + ".corrupt." + datetime.now().strftime("%Y%m%d_%H%M%S")
                )
                try:
                ...
                for zone_str, data in state_data["zone_phase_data"].items():
                    try:
                        ...
                        if zone_num < 1 or zone_num > self.num_zones:
                            self.log(
                                f"⚠️ Skipping invalid zone {zone_num} in state data",
                                level="WARNING",
                            )
                            continue
                        ...
                        self.zone_phase_data[zone_num] = {
                            "p0_start_time": (
                                datetime.fromisoformat(data["p0_start_time"])
                                if data.get("p0_start_time")
                                else None
                            ),
                            "p0_peak_vwc": data.get("p0_peak_vwc"),
                            "last_irrigation_time": (
                                datetime.fromisoformat(data["last_irrigation_time"])
                                if data.get("last_irrigation_time")
                                else None
                            ),
                        }
                        restored_count += 1
                        ...
                        self.log(
                            f"⚠️ Error restoring zone {zone_str} phase data: {e}",
                            level="WARNING",
                        )
                        continue
            ...
            if "zone_water_usage" in state_data:
                ...
                for zone_str, data in state_data["zone_water_usage"].items():
                    try:
                        ...
                        last_daily_reset = (
                            datetime.fromisoformat(data["last_reset_daily"]).date()
                            if data.get("last_reset_daily")
                            else today
                        )
                        last_weekly_reset = (
                            datetime.fromisoformat(data["last_reset_weekly"]).date()
                            if data.get("last_reset_weekly")
                            else today
                        )

                        # Only restore if from same day/week
                        daily_total = (
                            data.get("daily_total", 0.0)
                        ...
                        weekly_total = (
                            data.get("weekly_total", 0.0)
                            if (today - last_weekly_reset).days < 7
                            else 0.0
                        )
                        ...
                        self.log(
                            f"⚠️ Error restoring zone {zone_str} water usage: {e}",
                            level="WARNING",
                        )
                        continue
            ...
            if state_data.get("last_irrigation_time"):
                self.last_irrigation_time = datetime.fromisoformat(
                    state_data["last_irrigation_time"]
                )
                self.log(
                    f"✅ Restored last irrigation time: {self.last_irrigation_time}"
                )
            ...
            self.log(
                f"🔄 System recovered after {downtime.total_seconds():.0f} seconds of downtime"
            )
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                if phase and phase in ["P0", "P1", "P2", "P3"]:
                    ...
                    self.log(
                        f"✅ Zone {zone_num}: Restored phase {phase} from HA sensor"
                    )
        except Exception as e:
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                lights_on = self._are_lights_on(
                    current_time,
                    zone_schedule["lights_on"],
                    zone_schedule["lights_off"],
                )
                zone_phase = self.zone_phases.get(zone_num, "P2")

                self.log(
                    f"🔧 Zone {zone_num}: Time {current_time}, Lights ON: {zone_schedule['lights_on']}-{zone_schedule['lights_off']}, Currently: {'ON' if lights_on else 'OFF'}, Phase: {zone_phase}"
                )

                # If lights are on but zone isn't in P0, start P0 (morning dryback)
                if lights_on and zone_phase not in ["P0", "P1", "P2", "P3"]:
                    self.log(
                        f"🔧 Zone {zone_num}: Lights on but phase is {zone_phase}, starting P0 morning dryback"
                    )
                ...
                # If lights are on but zone is in P0 and dryback is done, fix it
                    ...
                    if zone_data.get("p0_start_time"):
                        elapsed = (
                            now - zone_data["p0_start_time"]
                        ).total_seconds() / 60
                        if elapsed > 120:  # More than 2 hours in P0
                            self.log(
                                f"🔧 Zone {zone_num}: Lights on and long P0 duration, moving to P1"
                            )
                ...
                # If lights are OFF, zone should be in P3 (waiting period before lights on)
                    ...
                    self.log(
                        f"🔧 Zone {zone_num}: Lights off but phase is {zone_phase}, correcting to P3 (waiting)"
                    )
    ...
    def _update_phase_sensors(self):
        ...
        try:
            ...
            for zone_num, phase in self.zone_phases.items():
                self.run_in(
                    self._async_set_entity_wrapper,
                    0,
    ...
    def _on_vwc_sensor_update(self, entity, attribute, old, new, kwargs):
        ...
        try:
            ...
            with self.lock:
                ...
                self.log(
                    f"🔍 DEBUG Emergency check: fusion={fusion_result['fused_value']:.1f}%"
                )
                # Schedule async emergency check
                self.run_in(
                    self._run_emergency_check, 0, vwc_value=fusion_result["fused_value"]
                )
        ...
        try:
            ...
            with self.lock:
                ...
                if ec_value > self.config["thresholds"]["critical_ec"]:
                    self.log(
                        f"🚨 Critical EC level detected: {ec_value:.2f} mS/cm",
                        level="WARNING",
                    )
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                await self._transition_zone_to_phase(
                    zone_num, new, "Manual phase change"
                )
    ...
    async def _on_crop_profile_change(self, entity, attribute, old, new, kwargs):
        ...
        try:
            ...
            if profile_result["status"] == "success":
                ...
                self.log(
                    f"❌ Failed to change crop profile: {profile_result.get('message', 'Unknown error')}"
                )
        ...
        try:
            ...
            with self.lock:
                ...
                if not profile_params:
                    self.log(
                        "⚠️ No active crop profile - using defaults", level="WARNING"
                    )
                    return
                ...
                # Get ML predictions
                ml_predictions = await self._get_ml_irrigation_predictions(
                    current_state
                )
                ...
                if decision["action"] == "irrigate":
                    ...
                    self.log(
                        f"⏳ Waiting: {decision.get('reason', 'Monitoring conditions')}"
                    )
    ...
    async def _get_current_system_state(self) -> Optional[Dict]:
        ...
        try:
            ...
            for sensor in self.config["sensors"]["vwc"]:
                try:
                    ...
                    if hasattr(value, "__await__"):
                        ...
                        continue
                    self.log(
                        f"🔍 DEBUG: Reading VWC sensor {sensor} = {value} (type: {type(value)})"
                    )
                    if value not in ["unavailable", "unknown", None, ""]:
                        ...
                        self.log(
                            f"⚠️ VWC sensor {sensor} unavailable: {value}",
                            level="WARNING",
                        )
                except Exception as e:
                    self.log(
                        f"❌ Error reading VWC sensor {sensor}: {e}", level="ERROR"
                    )
            ...
            for sensor in self.config["sensors"]["ec"]:
                try:
                    ...
                    if hasattr(value, "__await__"):
                        ...
                        continue
                    self.log(
                        f"🔍 DEBUG: Reading EC sensor {sensor} = {value} (type: {type(value)})"
                    )
                    if value not in ["unavailable", "unknown", None, ""]:
                        ...
                        self.log(
                            f"⚠️ EC sensor {sensor} unavailable: {value}",
                            level="WARNING",
                        )
                except Exception as e:
            ...
            try:
                temp_state = self.get_entity_value(
                    self.config["sensors"]["environmental"]["temperature"]
                )
                temperature = (
                    float(temp_state)
            ...
            try:
                hum_state = self.get_entity_value(
                    self.config["sensors"]["environmental"]["humidity"]
                )
                humidity = (
                    float(hum_state)
                    if hum_state not in ["unavailable", "unknown", None]
                    else 60.0
                )
            except (ValueError, TypeError):
                humidity = 60.0
            ...
            try:
                vpd_state = self.get_entity_value(
                    self.config["sensors"]["environmental"]["vpd"]
                )
                vpd = (
                    float(vpd_state)
                    if vpd_state not in ["unavailable", "unknown", None]
                    else 1.0
                )
            except (ValueError, TypeError):
                vpd = 1.0
            ...
            avg_vwc = (
                fused_vwc
                if fused_vwc is not None
                else (statistics.mean(list(vwc_sensors.values())) if vwc_sensors else 0)
            )
            avg_ec = (
                fused_ec
                if fused_ec is not None
                else (statistics.mean(list(ec_sensors.values())) if ec_sensors else 3.0)
            )

            self.log(
                f"DEBUG Fused values: VWC={avg_vwc:.2f}% (fusion: {fused_vwc}), EC={avg_ec:.2f} mS/cm (fusion: {fused_ec})"
            )
            self.log(f"DEBUG Raw VWC values: {list(vwc_sensors.values())}")
            self.log(f"DEBUG Raw EC values: {list(ec_sensors.values())}")
            self.log(
                f"DEBUG Active VWC sensors: {self.sensor_fusion.get_sensor_count_by_type('vwc')}"
            )
            self.log(
                f"DEBUG Active EC sensors: {self.sensor_fusion.get_sensor_count_by_type('ec')}"
            )
            ...
            return {
                ...
                "vpd": vpd,
                "zone_phases": self.zone_phases.copy(),
                "lights_on": self.get_entity_value(
                    "sun.sun", attribute="elevation", default=0
                )
                > 0,
                "timestamp": datetime.now(),
            }
    ...
    async def _get_ml_irrigation_predictions(
        ...
        try:
            ...
            features = {
                ...
                "steering_mode": self.get_entity_value(
                    "select.crop_steering_steering_mode", default="Vegetative"
                ),
            }
            ...
            if predictions.get("prediction_available", False):
                ...
                self.log(
                    f"🤖 ML predictions not available: {predictions.get('reason', 'Unknown')}"
                )
                return None
    ...
    async def _make_irrigation_decision(
        self,
        current_state: Dict,
        profile_params: Dict,
        dryback_status: Dict,
        ...
        try:
            ...
            phase_decision = self._evaluate_phase_requirements(
                current_state, profile_params
            )
            ...
            if dryback_status and dryback_status["dryback_in_progress"]:
                dryback_decision = self._evaluate_dryback_decision(
                    dryback_status, profile_params
                )
            ...
            if ml_predictions and ml_predictions.get("model_confidence", 0) > 0.7:
                ...
                if (
                    ml_decision["action"] == "irrigate"
                    and ml_decision["confidence"] > decision["confidence"]
                ):
            ...
            if decision["action"] == "wait":
                profile_decision = self._evaluate_profile_decision(
                    current_state, profile_params
                )
    ...
    def _evaluate_phase_requirements(
        self, current_state: Dict, profile_params: Dict
    ) -> Dict:
        ...
        for zone_num in range(1, self.num_zones + 1):
            ...
            if zone_profile != self.get_entity_value("select.crop_steering_crop_type"):
                ...
                zone_profile_params = self.crop_profiles.get_profile_parameters(
                    zone_profile
                )
            else:
            ...
            if zone_phase == "P0":  # Dryback phase - check for emergencies only
                ...
                if decision["needs_irrigation"]:
                    ...
                    zones_by_priority["Critical"].append(
                        zone_num
                    )  # P0 emergencies are always critical
        ...
        for priority in ["Critical", "High", "Normal", "Low"]:
            for zone_num in zones_by_priority[priority]:
                if (
                    zone_num not in all_zones_to_irrigate
                    and self._get_zone_group(zone_num) == "Ungrouped"
                ):
        ...
        if all_zones_to_irrigate:
            ...
            for zone_num in all_zones_to_irrigate:
                ...
                zone_details.append(
                    f"Z{zone_num}[{self.zone_phases[zone_num]}]:{decision['vwc']:.1f}%"
                )
        ...
        return {
            "action": "wait",
            "reason": "All zones satisfied in their current phases",
        }
    ...
    def _evaluate_zone_p1_needs(self, zone_num: int, profile_params: Dict) -> Dict:
        ...
        target_vwc = self._get_number_entity_value(
            "number.crop_steering_p1_target_vwc", 65
        )
        zone_vwc = self._get_zone_vwc(zone_num)
        zone_ec = self._get_zone_ec(zone_num)

        # Get growth stage for EC target selection
        growth_stage = self._get_select_entity_value(
            "select.crop_steering_growth_stage", "Vegetative"
        )

        # Get EC target for P1 based on growth stage
        if growth_stage.lower() == "vegetative":
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_veg_p1", 3.0
            )
        else:  # Generative
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_gen_p1", 5.0
            )

        # Get P1 progression parameters
        initial_shot_size = self._get_number_entity_value(
            "number.crop_steering_p1_initial_shot_size", 2.0
        )
        shot_increment = self._get_number_entity_value(
            "number.crop_steering_p1_shot_increment", 0.5
        )
        max_shot_size = self._get_number_entity_value(
            "number.crop_steering_p1_max_shot_size", 10.0
        )
        min_shots = self._get_number_entity_value(
            "number.crop_steering_p1_min_shots", 3.0
        )
        max_shots = self._get_number_entity_value(
            "number.crop_steering_p1_max_shots", 6.0
        )
        time_between_shots = self._get_number_entity_value(
            "number.crop_steering_p1_time_between_shots", 15.0
        )
        ...
        last_shot_time = (
            p1_data.shot_history[-1]["timestamp"] if p1_data.shot_history else None
        )
        ...
        # Check EC conditions
        ec_irrigation_decision = self._evaluate_ec_irrigation_need(
            zone_ec, ec_target, "P1"
        )

        # Determine if we need irrigation (VWC or EC driven)
        needs_irrigation = (
            vwc_needs_irrigation or ec_irrigation_decision["needs_irrigation"]
        )
    ...
    def _evaluate_zone_p2_needs(self, zone_num: int, profile_params: Dict) -> Dict:
        ...
        vwc_threshold = self._get_number_entity_value(
            "number.crop_steering_p2_vwc_threshold", 60
        )
        zone_vwc = self._get_zone_vwc(zone_num)
        zone_ec = self._get_zone_ec(zone_num)
        ...
        if growth_stage.lower() == "vegetative":
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_veg_p2", 3.2
            )
        else:  # Generative
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_gen_p2", 6.0
            )

        # Get P2 EC thresholds for ratio-based irrigation
        ec_high_threshold = self._get_number_entity_value(
            "number.crop_steering_p2_ec_high_threshold", 1.2
        )
        ec_low_threshold = self._get_number_entity_value(
            "number.crop_steering_p2_ec_low_threshold", 0.8
        )
        ...
        needs_irrigation = (
            vwc_needs_irrigation
            or ec_irrigation_decision["needs_irrigation"]
            or ec_ratio_decision["needs_irrigation"]
        )

        if needs_irrigation:
            # Calculate shot size with EC-based adjustments
            base_shot_size = self._get_number_entity_value(
    ...
    def _evaluate_zone_p3_needs(self, zone_num: int) -> Dict:
        ...
        emergency_threshold = self._get_number_entity_value(
        ...
        if growth_stage.lower() == "vegetative":
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_veg_p3", 3.0
            )
        else:  # Generative
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_gen_p3", 4.5
            )
        ...
        if vwc_emergency or ec_emergency:
            ...
            shot_size = self._get_number_entity_value(
                "number.crop_steering_p3_emergency_shot_size", 1.0
            )
    ...
    def _evaluate_zone_p0_needs(self, zone_num: int) -> Dict:
        ...
        if growth_stage.lower() == "vegetative":
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_veg_p0", 3.0
            )
        else:  # Generative
            ec_target = self._get_number_entity_value(
                "number.crop_steering_ec_target_gen_p0", 4.0
            )
        ...
        if ec_emergency:
            ...
            self._get_number_entity_value("number.crop_steering_ec_target_flush", 0.8)
    ...
    def _evaluate_dryback_decision(
        self, dryback_status: Dict, profile_params: Dict
    ) -> Dict:
        ...
        return {
            "action": "wait",
            "reason": f"Dryback in progress: {current_dryback:.1f}%",
        }
    ...
    def _evaluate_ml_decision(self, ml_predictions: Dict, current_state: Dict) -> Dict:
        ...
        return {
            "action": "wait",
            "reason": f"ML prediction: {max_need:.1%} need ({urgency})",
        }

    def _evaluate_profile_decision(
        self, current_state: Dict, profile_params: Dict
    ) -> Dict:
    ...
    async def _execute_intelligent_irrigation(self, decision: Dict):
        ...
        try:
            ...
            if zones:
                ...
                decision.get("shot_size_percent", 2.0)

                self.log(
                    f"🚨 Executing emergency irrigation: Zones {zones}, {duration}s - {reason}"
                )

                # Execute irrigation for each zone that needs it
                for zone in zones:
                    try:
                        zone_result = await self._execute_irrigation_shot(
                            zone, duration, shot_type="emergency"
                        )
                        ...
                        self.log(
                            f"❌ Error irrigating zone {zone}: {zone_error}",
                            level="ERROR",
                        )
            ...
            self.log(
                f"💧 Executing intelligent irrigation: Zone {zone}, {duration}s - {reason}"
            )

            # Execute irrigation
            irrigation_result = await self._execute_irrigation_shot(
                zone, duration, shot_type="intelligent"
            )
    ...
    async def _select_optimal_zone(self) -> Optional[int]:
        ...
        try:
            ...
            for zone in configured_zones:
                ...
                zone_enabled = self._get_switch_state(
                    f"switch.crop_steering_zone_{zone}_enabled", True
                )
                if not zone_enabled:
                    continue
            ...
            if not candidate_zones:
                self.log(
                    "⚠️ No zones available for irrigation (all blocked or disabled)"
                )
                return None
            ...
            if selected_zone:
                ...
                self.log(
                    f"🎯 Optimal zone selected: Zone {selected_zone} (Group: {zone_group}, Priority: {zone_priority}, VWC: {zone_vwc:.1f}%)"
                )
    ...
    async def _select_zone_by_vwc_fallback(
        self, candidate_zones: List[int]
    ) -> Optional[int]:
        ...
        try:
            ...
            for zone in candidate_zones:
                zone_vwc_sensors = [
                    s
                    for s in self.config["sensors"]["vwc"]
                    if f"r{zone}" in s or f"z{zone}" in s
                ]
                ...
                if zone_vwc_sensors:
                    ...
                    if zone_vwc_values:
                        ...
                        vwc_std = (
                            statistics.stdev(zone_vwc_values)
                            if len(zone_vwc_values) > 1
                            else 0
                        )
            ...
            if zone_scores:
                ...
                self.log(
                    f"🎯 Fallback zone selected: {optimal_zone} (VWC score: {zone_scores[optimal_zone]:.2f})"
                )
                return optimal_zone
    ...
    async def _execute_irrigation_shot(
        self, zone: int, duration: int, shot_type: str = "manual"
    ) -> Dict:
        ...
        try:
            ...
            zone_manual_override = self._get_switch_state(
                f"switch.crop_steering_zone_{zone}_manual_override", False
            )
            ...
            system_enabled = self._get_switch_state(
                "switch.crop_steering_system_enabled", True
            )
            auto_irrigation_enabled = self._get_switch_state(
                "switch.crop_steering_auto_irrigation_enabled", True
            )
            zone_enabled = self._get_switch_state(
                f"switch.crop_steering_zone_{zone}_enabled", True
            )
            ...
            try:
                ...
                if not pump_entity or not main_line_entity or not zone_entity:
                    ...
                    self.log(
                        f"🚨 Hardware configuration error: Missing entities {missing_entities}",
                        level="ERROR",
                    )
                    return {
                        "status": "error",
                ...
                self.log(
                    f"🚨 Hardware configuration error: Missing key {e}", level="ERROR"
                )
                return {
                    "status": "error",
            ...
            self.log(
                f"💧 Irrigation started: Zone {zone}, {duration}s, Type: {shot_type}"
            )
            ...
            irrigation_result = {
                ...
                "efficiency": (
                    min((post_vwc - pre_vwc) / duration * 100, 1.0)
                    if pre_vwc and post_vwc
                    else 0.5
                ),
            }
            ...
            if machine and machine.state.current_phase == IrrigationPhase.P1_RAMP_UP:
                ...
                machine
                and machine.state.current_phase == IrrigationPhase.P2_MAINTENANCE
            ):
                machine.record_p2_irrigation()
            elif (
                machine
                and machine.state.current_phase == IrrigationPhase.P3_PRE_LIGHTS_OFF
            ):
            ...
            self.log(
                f"✅ Irrigation completed: Zone {zone}, {actual_duration:.1f}s, "
    ...
    def _get_zone_average_vwc(self, zone: int) -> Optional[float]:
        ...
        try:
            ...
            if not vwc_sensors:
                self.log(
                    f"⚠️ No VWC sensors configured for zone {zone}", level="WARNING"
                )
                return None
            ...
            for sensor in zone_sensors:
                ...
                try:
                    ...
                    if value is not None and 0 <= value <= 100:  # Validate VWC range
                        ...
                        self.log(
                            f"⚠️ VWC value {value} from {sensor} out of range (0-100)",
                            level="WARNING",
                        )
                except Exception as e:
            ...
            if vwc_values:
                ...
                self.log(
                    f"Zone {zone} VWC: {avg_vwc:.1f}% from {len(vwc_values)} sensors",
                    level="DEBUG",
                )
                return avg_vwc
            else:
    ...
    async def _emergency_stop(self):
        ...
        try:
            ...
            for zone_id, zone_entity in zone_valves.items():
                if zone_entity:
                    try:
                        ...
                        self.log(
                            f"🛑 Emergency stop: Zone {zone_id} valve {zone_entity} turned off"
                        )
                    except Exception as e:
                        self.log(
                            f"⚠️ Emergency stop: Failed to turn off zone {zone_id} valve: {e}",
                            level="WARNING",
                        )
            ...
            if main_line_entity:
                try:
                    await self.call_service(
                        "switch/turn_off", entity_id=main_line_entity
                    )
                    self.log(
                        f"🛑 Emergency stop: Main line {main_line_entity} turned off"
                    )
                except Exception as e:
                    self.log(
                        f"⚠️ Emergency stop: Failed to turn off main line: {e}",
                        level="WARNING",
                    )
            ...
            if pump_entity:
                try:
                    ...
                    self.log(
                        f"⚠️ Emergency stop: Failed to turn off pump: {e}",
                        level="WARNING",
                    )

            self.irrigation_in_progress = False
            self.log(
                "🛑 Emergency stop executed - all available irrigation hardware turned off"
            )
    ...
    async def _check_emergency_conditions(self, fused_vwc: float):
        ...
        try:
            if fused_vwc and fused_vwc < self.config["thresholds"]["emergency_vwc"]:
                self.log(
                    f"🚨 Emergency VWC condition: {fused_vwc:.1f}%", level="WARNING"
                )
                ...
                if not auto_irrigation_enabled:
                    self.log(
                        "🛑 Emergency irrigation blocked: Auto irrigation disabled"
                    )
                    return
                ...
                if tank_filling:
                    self.log(
                        "🛑 Emergency irrigation blocked: Tank filling in progress"
                    )
                    return
                ...
                if time_since_last > cooldown_seconds:
                    ...
                    emergency_zone = (
                        await self._select_emergency_zone_from_integration()
                    )
                    if emergency_zone:
                        # Check if zone is enabled
                        zone_enabled = self._get_switch_state(
                            f"switch.crop_steering_zone_{emergency_zone}_enabled", True
                        )
                        if not zone_enabled:
                            self.log(
                                f"🛑 Emergency irrigation blocked: Zone {emergency_zone} disabled"
                            )
                            return
                        ...
                        if await self._should_abandon_emergency_zone(emergency_zone):
                            self.log(
                                f"🚫 Abandoning emergency irrigation for Zone {emergency_zone} - possible blocked dripper"
                            )
                            return

                        await self._execute_irrigation_shot(
                            emergency_zone, 60, shot_type="emergency"
                        )
                    else:
                        # Fallback to zone 3 based on user feedback
                        self.log(
                            "⚠️ Integration sensor selection failed, using Zone 3 fallback"
                        )
                        zone_3_enabled = self._get_switch_state(
                            "switch.crop_steering_zone_3_enabled", True
                        )
                        if zone_3_enabled:
                            await self._execute_irrigation_shot(
                                3, 60, shot_type="emergency"
                            )
                        else:
                    ...
                    self.log(
                        f"⏱️ Emergency irrigation on cooldown: {remaining_cooldown:.0f}s remaining"
                    )
    ...
    async def _select_emergency_zone_from_integration(self) -> Optional[int]:
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                try:
                    ...
                    if history and len(history) > 0 and len(history[0]) > 0:
                        ...
                        if hasattr(last_state, "state") and last_state.state not in [
                            "unknown",
                            "unavailable",
                        ]:
                ...
                if value is None:
                    try:
                        ...
                        if state_value not in [
                            ...
                            "unavailable",
                            None,
                        ] and not hasattr(state_value, "__await__"):
                            value = state_value
            ...
            if zone_vwc:
                ...
                self.log(
                    f"🚨 Emergency zone selected: Zone {emergency_zone} with {lowest_vwc:.1f}% VWC"
                )
                self.log(f"📊 All zone VWCs: {zone_vwc}")
                return emergency_zone
            else:
                self.log(
                    "⚠️ CRITICAL: All sensors unavailable - using user-specified Zone 3 override"
                )
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                zone_sensors = [
                    s
                    for s in self.config["sensors"]["vwc"]
                    if f"r{zone_num}" in s or f"z{zone_num}" in s
                ]
                zone_values = []
                ...
                for sensor in zone_sensors:
                    ...
                    try:
                        ...
                        if value not in [
                            ...
                            "unavailable",
                            None,
                        ] and not hasattr(value, "__await__"):
                ...
                if zone_values:
                    ...
                    self.log(
                        f"🔍 Zone {zone_num} emergency VWC: {avg_vwc:.1f}% (from {len(zone_values)} sensors)"
                    )
        ...
        try:
            ...
            if state not in ["unknown", "unavailable", None]:
                ...
                if hasattr(state, "__await__"):
                    self.log(
                        f"⚠️ Async context detected for {integration_sensor}, falling back to safe method"
                    )
                    ...
                    try:
                        ...
                        if self.entity_exists(integration_sensor):
                            ...
                            if (
                                alt_state
                                and not hasattr(alt_state, "__await__")
                                and alt_state not in ["unknown", "unavailable"]
                            ):
            ...
            if "sensors" in self.config and "vwc" in self.config["sensors"]:
                for sensor in self.config["sensors"]["vwc"]:
                    ...
                    if (
                        f"_zone_{zone_num}_" in sensor
                        or f"_z{zone_num}_" in sensor
                        or f"_r{zone_num}_" in sensor
                        or f"zone{zone_num}" in sensor.lower()
                    ):
                ...
                for sensor in zone_vwc_sensors:
                    try:
                        ...
                        if state not in ["unknown", "unavailable", None]:
                            ...
                            if hasattr(state, "__await__"):
                                self.log(
                                    f"⚠️ Async task from sensor {sensor}, trying alternative method"
                                )
    ...
    def _get_number_entity_value(self, entity_id: str, default: float) -> float:
        ...
        try:
            ...
            if not self.entity_exists(entity_id):
                self.log(
                    f"⚠️ Number entity {entity_id} does not exist, using default: {default}",
                    level="DEBUG",
                )
                return default
            ...
            if value < -1000 or value > 10000:
                self.log(
                    f"⚠️ Number entity {entity_id} has unreasonable value {value}, using default: {default}",
                    level="WARNING",
                )
                return default
            ...
            self.log(
                f"❌ Error reading number {entity_id}: {e}, using default: {default}"
            )
            return default
    ...
    def _get_select_entity_value(self, entity_id: str, default: str) -> str:
        ...
        try:
            ...
            if not self.entity_exists(entity_id):
                self.log(
                    f"⚠️ Select entity {entity_id} does not exist, using default: {default}",
                    level="DEBUG",
                )
                return default
            ...
            if hasattr(state, "__await__"):
                self.log(
                    f"⚠️ Select {entity_id} async Task detected, using default: {default}",
                    level="DEBUG",
                )
                return default
            ...
            if state not in ["unknown", "unavailable", None]:
                ...
                self.log(
                    f"⚠️ Select {entity_id} unavailable, using default: {default}",
                    level="DEBUG",
                )
                return default
        except Exception as e:
            self.log(
                f"❌ Error reading select {entity_id}: {e}, using default: {default}",
                level="WARNING",
            )
            return default
    ...
    def _get_zone_ec(self, zone_num: int) -> Optional[float]:
        ...
        try:
            zone_sensors = [
                s
                for s in self.config["sensors"]["ec"]
                if f"r{zone_num}" in s
                or f"z{zone_num}" in s
                or f"zone_{zone_num}" in s.lower()
            ]
            if not zone_sensors:
            ...
            for sensor in zone_sensors:
                try:
                    value = self.get_entity_value(sensor)
                    if value not in ["unknown", "unavailable", None] and not hasattr(
                        value, "__await__"
                    ):
    ...
    def _evaluate_ec_irrigation_need(
        self, current_ec: Optional[float], target_ec: float, phase: str
    ) -> Dict:
        ...
        try:
            ...
            ec_stacking_enabled = self._get_switch_state(
                "switch.crop_steering_ec_stacking_enabled", False
            )
    ...
    def _evaluate_p2_ec_ratio_irrigation(
        self,
    ...
    def _calculate_ec_adjusted_shot_size(
        self,
        base_shot_size: float,
        current_ec: Optional[float],
        target_ec: float,
        ec_decision: Dict,
    ) -> float:
    ...
    def _calculate_p1_progressive_shot_size(
        self,
        zone_num: int,
        initial_shot_size: float,
        shot_increment: float,
        max_shot_size: float,
        ...
        try:
            ...
            zone_multiplier = self._get_number_entity_value(
                f"number.crop_steering_zone_{zone_num}_shot_size_multiplier", 1.0
            )
            final_size = progressive_size * zone_multiplier

            self.log(
                f"📈 Zone {zone_num} P1 shot progression: base={initial_shot_size:.1f}% + increment={shot_increment:.1f}% * {current_shot_count} = {progressive_size:.1f}% * multiplier={zone_multiplier:.1f} = {final_size:.1f}%"
            )
            ...
            self.log(
                f"❌ Error calculating P1 progressive shot size: {e}", level="ERROR"
            )
            return initial_shot_size

    def _update_p1_progression_after_irrigation(
        self, zone_num: int, shot_size: float, vwc_before: Optional[float]
    ):
        ...
        try:
            ...
            # Log progression
            self.log(
                f"📊 Zone {zone_num} P1 progression updated: shot {zone_data['p1_shot_count']}, size {shot_size:.1f}%, VWC before: {vwc_before:.1f}%"
            )
    ...
    def _reset_p1_progression(self, zone_num: int):
        ...
        try:
            ...
            if start_time:
                ...
                self.log(
                    f"🎯 Zone {zone_num} P1 completed: {shot_count} shots over {duration:.1f} minutes"
                )
    ...
    def _get_number_of_zones(self) -> int:
        ...
        try:
            ...
            zones_entity = self.get_entity_value(
                "number.crop_steering_substrate_volume"
            )  # This exists, so integration is loaded
            if zones_entity:
    ...
    def _calculate_next_irrigation_time(self) -> datetime | None:
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                if zone_phase == "P0":
                    ...
                    if zone_vwc is not None:
                        threshold = self._get_number_entity_value(
                            "number.crop_steering_p2_vwc_threshold", 60.0
                        )
                    ...
                    emergency_threshold = self._get_number_entity_value(
                        "number.crop_steering_p3_emergency_vwc_threshold", 45
                    )
                ...
                if zone_next_time and (
                    earliest_time is None or zone_next_time < earliest_time
                ):
    ...
    async def _check_phase_transitions(self, kwargs):
        ...
        try:
            ...
            p0_max_duration = self._get_number_entity_value(
                "number.crop_steering_p0_max_wait_time", 45
            )
            p1_recovery_target = self._get_number_entity_value(
                "number.crop_steering_p1_target_vwc", 65
            )
            dryback_target = self._get_number_entity_value(
                "number.crop_steering_veg_dryback_target", 50
            )
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                lights_on = self._are_lights_on(
                    current_time, lights_on_time, lights_off_time
                )
                ...
                if lights_on and current_phase != "P0":
                    ...
                    target_phase = "P0"
                    reason = (
                        f"Zone {zone_num}: Lights on - starting morning dryback phase"
                    )

                elif lights_on and current_phase == "P0":
                    # Check P0 → P1 transition conditions for this zone
                    if self._should_zone_exit_p0(
                        zone_num, zone_vwc, dryback_target, p0_max_duration
                    ):
                        target_phase = "P1"
                    ...
                    should_transition = await self._should_zone_start_p3(
                        zone_num, lights_on_time, lights_off_time
                    )
    ...
    def _are_lights_on(
        self, current_time: time, lights_on: time, lights_off: time
    ) -> bool:
    ...
    def _time_is_near(
        self, current: time, target: time, tolerance_minutes: int = 5
    ) -> bool:
    ...
    def _should_zone_exit_p0(
        self,
        zone_num: int,
        zone_vwc: float,
        dryback_target: float,
        ...
        try:
            ...
            min_wait_time = self._get_number_entity_value(
                "number.crop_steering_p0_min_wait_time", 30.0
            )
            dryback_drop_percent = self._get_number_entity_value(
                "number.crop_steering_p0_dryback_drop_percent", 15.0
            )

            # Check if P0 phase has been initialized
            if not zone_data.get("p0_start_time"):
                self.log(
                    f"⚠️ Zone {zone_num}: P0 not properly initialized - triggering initialization"
                )
            ...
            if elapsed_minutes < min_wait_time:
                ...
                self.log(
                    f"⏱️ Zone {zone_num}: P0 minimum wait: {remaining_time:.1f}min remaining (min: {min_wait_time}min)"
                )
                return False
            ...
            if elapsed_minutes >= max_duration_minutes:
                self.log(
                    f"🚨 Zone {zone_num}: P0 max duration exceeded: {elapsed_minutes:.1f}min >= {max_duration_minutes}min - forcing P1 transition"
                )
                return True
            ...
            if zone_vwc and zone_data.get("p0_peak_vwc"):
                ...
                dryback_percent = (
                    (dryback_amount / peak_vwc) * 100 if peak_vwc > 0 else 0
                )

                # Check against target dryback percentage
                if dryback_percent >= dryback_target:
                    self.log(
                        f"🎯 Zone {zone_num}: P0 dryback target achieved: {dryback_percent:.1f}% >= {dryback_target}% (dropped {dryback_amount:.1f}% from peak {peak_vwc:.1f}%)"
                    )
                    return True
                ...
                if dryback_amount >= dryback_drop_percent:
                    self.log(
                        f"📉 Zone {zone_num}: P0 minimum drop achieved: {dryback_amount:.1f}% >= {dryback_drop_percent}% (from peak {peak_vwc:.1f}%)"
                    )
                    return True
                ...
                self.log(
                    f"📊 Zone {zone_num}: P0 progress - {elapsed_minutes:.1f}min elapsed, {dryback_percent:.1f}% dried (need {remaining_target:.1f}% more or {remaining_drop:.1f}% drop)"
                )

            # INTELLIGENT DRYBACK RATE CHECK - Predict completion time
            if self._should_p0_exit_based_on_rate(
                zone_num, zone_vwc, dryback_target, max_duration_minutes
            ):
                return True
    ...
    def _initialize_p0_phase(self, zone_num: int, current_vwc: Optional[float]):
        ...
        try:
            ...
            if current_vwc is not None:
                ...
                self.log(
                    f"🌱 Zone {zone_num}: P0 initialized - peak VWC: {current_vwc:.1f}%, start time: {now.strftime('%H:%M')}"
                )
            else:
                # Estimate peak VWC if no current reading
                zone_data["p0_peak_vwc"] = 75.0  # Conservative estimate
                self.log(
                    f"⚠️ Zone {zone_num}: P0 initialized without VWC reading - using estimated peak: 75.0%"
                )
    ...
    def _should_p0_exit_based_on_rate(
        self,
        zone_num: int,
        ...
        try:
            ...
            if total_estimated_duration > max_duration_minutes * 0.9:  # 90% of max time
                self.log(
                    f"⏰ Zone {zone_num}: P0 rate-based exit - would exceed max duration: {total_estimated_duration:.1f}min > {max_duration_minutes * 0.9:.1f}min"
                )
                return True

            # If rate is extremely slow, exit to prevent stalling
            if (
                dryback_rate < 0.1 and elapsed_minutes > 60
            ):  # Less than 0.1%/min after 1 hour
                self.log(
                    f"🐌 Zone {zone_num}: P0 slow rate exit - dryback rate too slow: {dryback_rate:.3f}%/min"
                )
                return True
            ...
            self.log(
                f"❌ Error checking zone {zone_num} P0 exit conditions: {e}",
                level="ERROR",
            )
            return False
    ...
    async def _calculate_optimal_p3_timing(self, lights_off_time: time) -> time | None:
        ...
        try:
            ...
            target_dryback = self._get_number_entity_value(
                "number.crop_steering_veg_dryback_target", 50
            )
            ...
            if hasattr(self, "dryback_detector") and self.dryback_detector:
                ...
                prediction = await self.dryback_detector.predict_target_dryback_time(
                    target_dryback
                )

                if prediction.get("prediction_available"):
                    predicted_minutes = prediction.get(
                        "predicted_minutes_remaining", 60
                    )
                    confidence = prediction.get("confidence", 0.5)

                    self.log(
                        f"🧠 ML Dryback Prediction: {predicted_minutes:.1f}min to reach {target_dryback}% (confidence: {confidence:.2f})"
                    )
                    ...
                    optimal_p3_start = lights_off_dt - timedelta(
                        minutes=predicted_minutes + 30
                    )  # +30min buffer for P3 irrigation
            ...
            fallback_dt = datetime.combine(
                datetime.today(), lights_off_time
            ) - timedelta(hours=1)
            return fallback_dt.time()

    async def _calculate_historical_p3_timing(
        ...
        try:
            ...
            substrate_volume = self._get_number_entity_value(
                "number.crop_steering_substrate_volume", 10
            )
            ...
            estimated_dryback_minutes = (
                base_dryback_minutes * volume_factor * target_factor
            )
            ...
            self.log(
                f"📊 Historical P3 timing: {total_minutes_needed:.0f}min before lights off (est. dryback: {estimated_dryback_minutes:.0f}min)"
            )
    ...
    async def _should_zone_start_p3(
        self, zone_num: int, lights_on_time: time, lights_off_time: time
    ) -> bool:
        ...
        try:
            ...
            if predicted_dryback_rate is None:
                ...
                predicted_dryback_rate = (
                    2.0 / substrate_volume
                )  # %/hour, smaller = slower
            ...
            if hours_until_lights_on <= hours_needed_for_dryback:
                self.log(
                    f"🎯 Zone {zone_num}: Time to start P3 dryback. "
            ...
            if zone_data.get("last_irrigation_time"):
                time_since_irrigation = (
                    now - zone_data["last_irrigation_time"]
                ).total_seconds() / 60
                if time_since_irrigation < 5:  # Just irrigated in last 5 minutes
                    ...
                    hours_until_dry = (
                        vwc_buffer / predicted_dryback_rate
                    ...
                    if hours_until_dry >= hours_until_lights_on:
                        self.log(
                            f"💧 Zone {zone_num}: Last P2 irrigation complete. "
            ...
            self.log(
                f"❌ Error checking zone {zone_num} P3 transition: {e}", level="ERROR"
            )
            return False
    ...
    async def _get_zone_dryback_rate(self, zone_num: int) -> float | None:
        ...
        try:
            ...
            self.log(
                f"❌ Error getting zone {zone_num} dryback rate: {e}", level="ERROR"
            )
            return None

    async def _transition_zone_to_phase(
        self, zone_num: int, new_phase: str, reason: str
    ):
        ...
        try:
            ...
            transition_type = (
                PhaseTransition.MANUAL_OVERRIDE
            )  # Default for manual changes
            current_phase = machine.state.current_phase

            # Map specific transitions
            if (
                current_phase == IrrigationPhase.P0_MORNING_DRYBACK
                and target_phase == IrrigationPhase.P1_RAMP_UP
            ):
                ...
                current_phase == IrrigationPhase.P1_RAMP_UP
                and target_phase == IrrigationPhase.P2_MAINTENANCE
            ):
            ...
            if not success:
                self.log(
                    f"Zone {zone_num}: Failed to transition to {new_phase}",
                    level="WARNING",
                )
                return
            ...
            self.log(
                f"❌ Error transitioning zone {zone_num} to phase {new_phase}: {e}",
                level="ERROR",
            )
    ...
    async def _check_all_zone_phase_transitions(self):
        ...
        try:
            ...
            lights_on_time = self._get_zone_schedule(1)[
                "lights_on"
            ]  # System-wide schedule
            self._get_zone_schedule(1)["lights_off"]
            ...
            lights_just_on = self._is_time_between(
                ...
                lights_on_time,
                (
                    datetime.combine(datetime.today(), lights_on_time)
                    + timedelta(minutes=5)
                ).time(),
            )
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                if (
                    lights_just_on
                    and current_phase == IrrigationPhase.P3_PRE_LIGHTS_OFF
                ):
                    await self._transition_zone_to_phase(
                        zone_num, "P0", "Lights on - starting morning dryback"
                    )
                ...
                # P0 → P1: Dryback complete or timeout
                    ...
                    if p0_data and zone_vwc is not None:
                        ...
                        machine.update_p0_dryback(
                            zone_vwc, p0_data.peak_vwc or zone_vwc
                        )
                        ...
                        phase_duration = (
                            machine.get_phase_duration().total_seconds() / 60
                        )  # minutes
                        ...
                        if current_dryback >= dryback_target:
                            await self._transition_zone_to_phase(
                                zone_num,
                                "P1",
                                f"Dryback complete: {current_dryback:.1f}%",
                            )
                        elif phase_duration >= p0_data.max_duration_minutes:
                            await self._transition_zone_to_phase(
                                zone_num,
                                "P1",
                                f"P0 timeout: {phase_duration:.0f} minutes",
                            )
                ...
                # P1 → P2: Recovery complete
                    ...
                    if p1_data and zone_vwc is not None:
                        if zone_vwc >= p1_data.target_vwc:
                            await self._transition_zone_to_phase(
                                zone_num, "P2", f"Recovery complete: {zone_vwc:.1f}%"
                            )
                ...
                # P2 → P3: ML-based or time-based transition
                    ...
                    if should_start_p3:
                        await self._transition_zone_to_phase(
                            zone_num, "P3", "ML predicted lights-off approaching"
                        )
    ...
    async def _should_zone_start_p3(self, zone_num: int) -> bool:
        ...
        try:
            ...
            time_until_lights_off = (
                lights_off_datetime - now
            ...
            if dryback_rate and dryback_rate > 0:
                ...
                target_dryback = self._get_number_entity_value(
                    "number.crop_steering_veg_dryback_target", 50.0
                )
            ...
            self.log(
                f"Error checking P3 transition for zone {zone_num}: {e}", level="ERROR"
            )
            return False
    ...
    async def _update_crop_profile_performance(self, irrigation_result: Dict):
        ...
        try:
            ...
            performance_data = {
                ...
                "target_achieved": irrigation_result.get("vwc_improvement", 0)
                > 3,  # 3% improvement considered success
                "water_used": irrigation_result.get("duration_actual", 30),
                "plant_response_score": min(
                    irrigation_result.get("vwc_improvement", 0) / 10, 1.0
                ),
            }
            ...
            if (
                update_result.get("adaptations_applied", {}).get("status")
                == "adaptations_applied"
            ):
    ...
    async def _monitor_sensor_health(self, kwargs):
        ...
        try:
            ...
            if faulty_sensors > 0 or offline_sensors > 0:
                self.log(
                    f"⚠️ Sensor issues: {faulty_sensors} faulty, {offline_sensors} offline",
                    level="WARNING",
                )
            ...
            for sensor_id, sensor_data in health_report["sensors"].items():
                ...
                self.set_entity_value(
                    entity_id, sensor_data["health_status"], attributes=sensor_data
                )
    ...
    async def _update_performance_analytics(self, kwargs):
        ...
        try:
            ...
            if health_report["total_sensors"] > 0:
                metrics["sensor_fusion_confidence"] = (
                    health_report["healthy_sensors"] / health_report["total_sensors"]
                )

            # Dryback detection
            dryback_status = self._get_latest_dryback_status()
            if dryback_status:
                metrics["dryback_detection_accuracy"] = dryback_status.get(
                    "confidence_score", 0
                )
            ...
            self.log(
                f"📊 Performance updated - ML: {metrics['ml_model_accuracy']:.2f}, "
    ...
    def _update_sensor_fusion_entities(self, sensor_id: str, fusion_result: Dict):
        ...
        try:
            ...
            self.run_in(
                ...
                attributes={
                    "health": fusion_result["sensor_health"],
    ...
    async def _update_phase_parameters(self):
        ...
        try:
            ...
            if current_params:
                ...
                self.log(
                    f"📊 Phase parameters updated for zones: {list(self.zone_phases.values())}"
                )
    ...
    def _get_switch_state(self, entity_id: str, default: bool = False) -> bool:
        ...
        try:
            ...
            self.log(
                f"🔍 DEBUG: Switch {entity_id} state: {state} (type: {type(state)})"
            )
            if state in ["on", True, "true", "1"]:
                ...
                self.log(
                    f"⚠️ Switch {entity_id} unknown state: {state}, using default: {default}"
                )
                return default
        except Exception as e:
            self.log(
                f"❌ Error reading switch {entity_id}: {e}, using default: {default}"
            )
            return default
    ...
    async def _track_emergency_attempt(self, zone_num: int, vwc_before: float):
        ...
        try:
            ...
            self.log(
                f"📊 Zone {zone_num} emergency attempts in last hour: {len(attempts)}"
            )
    ...
    async def _should_abandon_emergency_zone(self, zone_num: int) -> bool:
        ...
        try:
            ...
            recent_attempts = [
                attempt for attempt in attempts if attempt[0] > recent_cutoff
            ]

            # If we've had 4+ emergency shots in 30 minutes, consider abandoning
            if len(recent_attempts) >= 4:
                self.log(
                    f"🚫 Zone {zone_num}: {len(recent_attempts)} emergency shots in 30min - likely blocked dripper"
                )

                # Abandon for 2 hours
                self.emergency_attempts[zone_num]["abandoned_until"] = now + timedelta(
                    hours=2
                )
    ...
    async def _on_manual_irrigation_shot(self, event_name, data, kwargs):
        ...
        try:
            ...
            self.log(
                f"🔧 Manual irrigation shot requested: Zone {zone}, {duration}s, type: {shot_type}"
            )

            # Execute the irrigation shot (manual shots bypass some checks)
            result = await self._execute_irrigation_shot(
                zone, duration, shot_type="manual"
            )
            ...
            if result["status"] == "completed":
                ...
                self.log(
                    f"🛑 Manual irrigation shot blocked: Zone {zone} - {result['message']}"
                )
            else:
                self.log(
                    f"❌ Manual irrigation shot failed: Zone {zone} - {result.get('message', 'Unknown error')}"
                )
    ...
    async def _on_manual_override_event(self, event_name, data, kwargs):
        ...
        try:
            ...
            self.log(
                f"🔧 Manual override event: Zone {zone}, Action: {action}, Timeout: {timeout_minutes}min"
            )
    ...
    async def _enable_manual_override_with_timeout(
        self, zone: int, timeout_minutes: int
    ):
        ...
        try:
            ...
            self.log(
                f"🕐 Manual override enabled for Zone {zone} with {timeout_minutes} minute timeout"
            )
            ...
            self.log(
                f"❌ Error enabling manual override with timeout: {e}", level="ERROR"
            )
    ...
    async def _disable_manual_override(self, zone: int):
        ...
        try:
            ...
            # Update tracking
            self.manual_overrides[zone].update(
                {"timeout_handle": None, "enabled_time": None, "timeout_minutes": None}
            )
    ...
    async def _auto_disable_manual_override(self, kwargs):
        ...
        try:
            ...
            self.log(
                f"⏰ Manual override auto-disabled for Zone {zone} - timeout expired"
            )
    ...
    async def _on_phase_transition_service(self, event_name, data, kwargs):
        ...
        try:
            ...
            self.log(
                f"📡 Service: Phase transition to {target_phase} requested - {reason}"
            )
            ...
            for zone_num, machine in self.zone_state_machines.items():
                if forced:
                    ...
                    if success:
                        ...
                        self.log(
                            f"Zone {zone_num}: Could not transition to {target_phase} - conditions not met",
                            level="WARNING",
                        )
    ...
    async def _on_set_manual_override_service(self, event_name, data, kwargs):
        ...
        try:
            ...
            self.log(
                f"📡 Service: Manual override for Zone {zone} - enable={enable}, timeout={timeout_minutes}"
            )

            if enable:
                if timeout_minutes:
                    await self._enable_manual_override_with_timeout(
                        zone, timeout_minutes
                    )
                else:
                    await self._enable_manual_override_permanent(zone)
            else:
    ...
    def get_system_status(self) -> Dict:
        ...
        try:
            return {
                ...
                "ml_status": (
                    self.ml_predictor.get_model_status() if self.ml_predictor else {}
                ),
                "sensor_health": (
                    self.sensor_fusion.get_sensor_health_report()
                    if self.sensor_fusion
                    else {}
                ),
                "active_crop_profile": (
                    self.crop_profiles.current_profile if self.crop_profiles else None
                ),
                "emergency_attempts": {
                    zone: {
                        "recent_attempts": len(
                            [
                                ...
                                if a[0] > datetime.now() - timedelta(hours=1)
                            ]
                        ),
                        "abandoned_until": (
                            data["abandoned_until"].isoformat()
                            if data["abandoned_until"]
                            else None
                        ),
                    }
        ...
        try:
            ...
            if group_state and group_state not in ["unknown", "unavailable"]:
                ...
                if hasattr(group_state, "__await__"):
                    self.log(
                        f"⚠️ Async task detected for zone {zone_num} group, using default"
                    )
                    return "Ungrouped"
                return group_state
            return "Ungrouped"
        ...
        try:
            ...
            if priority_state and priority_state not in ["unknown", "unavailable"]:
                ...
                if hasattr(priority_state, "__await__"):
                    self.log(
                        f"⚠️ Async task detected for zone {zone_num} priority, using default"
                    )
                    return "Normal"
    ...
    def _get_zones_by_group(self, group_name: str) -> List[int]:
        ...
        try:
            ...
            self.log(
                f"❌ Error getting zones in group {group_name}: {e}", level="ERROR"
            )
            return []
    ...
    def _get_zones_by_priority(self, priority: str) -> List[int]:
        ...
        try:
            ...
            self.log(
                f"❌ Error getting zones at priority {priority}: {e}", level="ERROR"
            )
            return []
    ...
    def _sort_zones_by_priority(self, zone_list: List[int]) -> List[int]:
        ...
        try:
            return sorted(
                zone_list,
                key=lambda z: self._get_priority_score(self._get_zone_priority(z)),
                reverse=True,
            )
        except Exception as e:
        ...
        try:
            ...
            for zone in zones_in_group:
                ...
                if zone_valve_entity:
                    ...
                    if valve_state == "on":
                        self.log(
                            f"🚦 Group {group_name}: Zone {zone} already irrigating - blocking other zones in group"
                        )
                        return False
            ...
            self.log(
                f"❌ Error coordinating group {group_name} irrigation: {e}",
                level="ERROR",
            )
            return True  # Allow irrigation on error to prevent system lockup

    def _select_next_zone_by_priority(
        self, candidate_zones: List[int]
    ) -> Optional[int]:
        ...
        try:
            ...
            for zone in candidate_zones:
                ...
                self.log(
                    f"🎯 Zone {zone}: Priority={priority} ({priority_score}), VWC={zone_vwc:.1f}% (need={vwc_need_score:.2f}), Phase={zone_phase} (urgency={phase_urgency:.2f}) → Score={total_score:.2f}"
                )
            ...
            self.log(
                f"🏆 Selected Zone {selected_zone} with highest priority score: {max_score:.2f}"
            )
            return selected_zone
    ...
    async def _check_zone_conflicts(self, target_zone: int) -> bool:
        ...
        try:
            ...
            if zone_group != "Ungrouped":
                ...
                if not group_clear:
                    self.log(
                        f"🚫 Zone {target_zone} irrigation blocked: Group {zone_group} conflict"
                    )
            ...
            if active_irrigation_count >= max_concurrent_zones:
                self.log(
                    f"🚫 Zone {target_zone} irrigation blocked: {active_irrigation_count} zones already irrigating (max: {max_concurrent_zones})"
                )
                return True  # Conflict detected
            ...
            self.log(
                f"❌ Error checking zone {target_zone} conflicts: {e}", level="ERROR"
            )
            ...
            for group_name in all_groups:
                ...
                if zones_in_group:
                    ...
                    for zone in zones_in_group:
                        ...
                        # Irrigation status
                        zone_valve_entity = self.config["hardware"]["zone_valves"].get(
                            zone
                        )
                        if zone_valve_entity:
                            valve_state = self.get_entity_value(zone_valve_entity)
                    ...
                    group_status[group_name] = {
                        ...
                        "avg_vwc": (
                            round(sum(group_vwc_values) / len(group_vwc_values), 1)
                            if group_vwc_values
                            else None
                        ),
    ...
    async def _calculate_system_analytics(self) -> Dict:
        ...
        try:
            ...
            analytics["system_performance"] = {
                "uptime_hours": (now - getattr(self, "start_time", now)).total_seconds()
                / 3600,
                ...
                "active_zones": len(
                    [
                        ...
                        if self._get_switch_state(
                            f"switch.crop_steering_zone_{z}_enabled", True
                        )
                    ]
                ),
                "zones_irrigating": len(
                    [
                        z
                        for z in range(1, self.num_zones + 1)
            ...
            analytics["irrigation_analytics"] = (
                await self._calculate_irrigation_analytics()
            )
    ...
    async def _calculate_zone_analytics(self, zone_num: int) -> Dict:
        ...
        try:
            ...
            daily_water = self.zone_water_usage.get(zone_num, {}).get(
                "daily_total", 0.0
            )
            weekly_water = self.zone_water_usage.get(zone_num, {}).get(
                "weekly_total", 0.0
            )
            ...
            self.log(
                f"❌ Error calculating zone {zone_num} analytics: {e}", level="ERROR"
            )
            return {}
    ...
    def _calculate_zone_efficiency(self, zone_num: int) -> float:
        ...
        try:
            ...
            recent_shots = [
                shot
                for shot in p1_history
                if shot[0] > datetime.now() - timedelta(days=1)
            ]
            ...
            return (
                round(sum(efficiency_scores) / len(efficiency_scores), 2)
            ...
            self.log(
                f"❌ Error calculating zone {zone_num} efficiency: {e}", level="ERROR"
            )
            return 0.5
    ...
    async def _calculate_irrigation_analytics(self) -> Dict:
        ...
        try:
            total_daily_water = sum(
                [data.get("daily_total", 0) for data in self.zone_water_usage.values()]
            )
            total_weekly_water = sum(
                [data.get("weekly_total", 0) for data in self.zone_water_usage.values()]
            )
            total_daily_count = sum(
                [data.get("daily_count", 0) for data in self.zone_water_usage.values()]
            )
            ...
            for phase in ["P0", "P1", "P2", "P3"]:
                count = sum(
                    [1 for z_phase in self.zone_phases.values() if z_phase == phase]
                )
            ...
            return {
                ...
                "irrigation_frequency": round(
                    total_daily_count / max(self.num_zones, 1), 1
                ),
    ...
    def _calculate_sensor_analytics(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "overall_sensor_health": round(
                    (vwc_availability + ec_availability) / 2, 1
                ),
    ...
    async def _calculate_efficiency_metrics(self) -> Dict:
        ...
        try:
            ...
            for zone_data in self.emergency_attempts.values():
                recent_attempts = [
                    a
                    for a in zone_data["attempts"]
                    if a[0] > datetime.now() - timedelta(days=1)
                ]
            ...
            return {
                ...
                "system_efficiency_score": round(
                    (water_efficiency * 10 + automation_rate) / 2, 1
                ),
            }
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                if zone_vwc is not None:
                    ...
                    predictions[f"zone_{zone_num}_next_irrigation_hours"] = round(
                        next_irrigation_hours, 1
                    )
            ...
            if avg_vwc:
                vwc_trend = (
                    "stable"  # Simplified - could be enhanced with historical data
                )
                if avg_vwc < 50:
                    vwc_trend = "declining"
                elif avg_vwc > 70:
                ...
                predictions["estimated_daily_water_need"] = round(
                    self.num_zones * 5.0, 1
                )  # Simplified estimate
    ...
    def _update_analytics_entities(self, analytics_data: Dict):
        ...
        try:
            ...
            self.set_entity_value(
                "sensor.crop_steering_daily_water_usage",
            ...
            self.set_entity_value(
                "sensor.crop_steering_system_efficiency",
            ...
            for zone_key, zone_data in zone_analytics.items():
                ...
                self.set_entity_value(
                    f"sensor.crop_steering_zone_{zone_num}_health_score",
                ...
                self.set_entity_value(
                    f"sensor.crop_steering_zone_{zone_num}_efficiency",
    ...
    def _calculate_system_health_score(self) -> float:
        ...
        try:
            ...
            for sensor in self.config["sensors"]["vwc"]:
                if self.get_entity_value(sensor) not in [
                    "unknown",
                    "unavailable",
                    None,
                ]:
                    vwc_sensors_working += 1
            sensor_health = vwc_sensors_working / max(
                len(self.config["sensors"]["vwc"]), 1
            )
    ...
    async def _update_performance_metrics(self, analytics_data: Dict):
        ...
        try:
            ...
            self.log(
                f"📊 Performance Update - Health: {perf.get('system_health_score', 0):.1f}%, "
    ...
    async def _update_efficiency_metrics(self, analytics_data: Dict):
        ...
        try:
            ...
            self.set_entity_value(
                "sensor.crop_steering_water_efficiency",
        ...
        try:
            ...
            for key, value in predictions.items():
                if isinstance(value, (int, float)):
                    self.set_entity_value(
                        f"sensor.crop_steering_prediction_{key}",
                        state=value,
                        attributes={
                            "updated": datetime.now().isoformat(),
    ...
    def _check_irrigation_safety_limits(self, zone: int, shot_type: str) -> Dict:
        ...
        try:
            ...
            field_capacity = self._get_number_entity_value(
                "number.crop_steering_field_capacity", 80.0
            )
            max_ec_limit = self._get_number_entity_value(
                "number.crop_steering_max_ec", 8.0
            )

            # Get zone-specific daily limits
            max_daily_volume = self._get_number_entity_value(
                f"number.crop_steering_zone_{zone}_max_daily_volume", 20.0
            )
            ...
            daily_water_used = self.zone_water_usage.get(zone, {}).get(
                "daily_total", 0.0
            )
    ...
    def _check_irrigation_frequency_safety(self, zone: int) -> Dict:
        ...
        try:
            ...
            self.log(
                f"❌ Error checking irrigation frequency safety: {e}", level="ERROR"
            )
            return {"blocked": False}

    def _check_phase_specific_safety(
        self, zone: int, zone_vwc: Optional[float], zone_ec: Optional[float]
    ) -> Dict:
        """Check phase-specific safety limits."""
        try:
            zone_phase = self.zone_phases.get(zone, "P2")
            ...
            if zone_phase == "P1" and zone_vwc is not None:
                p1_target = self._get_number_entity_value(
            ...
            if zone_phase == "P3":
                ...
                emergency_threshold = self._get_number_entity_value(
                    "number.crop_steering_p3_emergency_vwc_threshold", 45.0
                )
                if (
                    zone_vwc is not None and zone_vwc > emergency_threshold + 5
                ):  # 5% buffer
                    return {
                        "blocked": True,
    ...
    def _update_safety_status_entities(self):
        ...
        try:
            ...
            for zone_num in range(1, self.num_zones + 1):
                ...
                self.set_entity_value(
                    f"sensor.crop_steering_zone_{zone_num}_safety_status",
                    state=status,
                    attributes={
                        "vwc": zone_vwc,
                        "ec": zone_ec,
            ...
            warning_zones = len(
                [
                    ...
                    for status in safety_status.values()
                    if status["status"]
                    in ["approaching_saturation", "approaching_ec_limit"]
                ]
            )
        ...
        try:
            ...
            try:
                ...
                if main_line_entity:
                    ...
                    self.log(
                        f"🛑 Emergency stop: Turned off main line {main_line_entity}"
                    )
                else:
                ...
                if zone_valves:
                    for zone_id, zone_valve in zone_valves.items():
                        if zone_valve:
                            ...
                            self.log(
                                f"🛑 Emergency stop: Turned off zone {zone_id} valve {zone_valve}"
                            )
                else:
