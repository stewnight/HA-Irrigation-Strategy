...
class IntelligentCropProfiles:
    ...
    def __init__(self, profiles_file: str = None):
        ...
        _LOGGER.info(
            f"Intelligent Crop Profiles initialized with {len(self.base_profiles)} base profiles"
        )
    ...
    def select_profile(
        self, profile_name: str, growth_stage: str = "vegetative"
    ) -> Dict:
        """
        ...
        # Check custom profiles
            ...
            return {
                ...
                "available_profiles": list(self.base_profiles.keys())
                + list(self.custom_profiles.keys()),
            }
    ...
    def get_current_parameters(self) -> Optional[Dict]:
        ...
        profile_source = (
            self.base_profiles
            if self.current_profile in self.base_profiles
        ...
        environmental_params = self._apply_environmental_adjustments(
            adapted_params, profile
        )
    ...
    def _apply_adaptations(self, base_params: Dict) -> Dict:
        ...
        if (
            not self.current_profile
            or self.current_profile not in self.adaptation_learning
        ):
            return base_params
    ...
    def update_performance(
        self, irrigation_result: Dict, environmental_data: Dict = None
    ) -> Dict:
        """
        ...
        return {
            ...
            "performance_samples": len(
                self.profile_performance_history[self.current_profile]
            ),
    ...
    def _learn_adaptations(self, performance_entry: Dict) -> Dict:
        ...
        avg_efficiency = self._mean(
            [p["irrigation_efficiency"] for p in recent_performance]
        )
        avg_response = self._mean([p["vwc_response"] for p in recent_performance])
        target_hit_rate = self._mean(
            [p["target_achievement"] for p in recent_performance]
        )
        ...
        for param, adjustment in adaptations.items():
            current_adaptation = self.adaptation_learning[self.current_profile].get(
                param, 0
            )
        ...
        _LOGGER.info(
            f"Profile adaptations updated for {self.current_profile}: {adaptations}"
        )
    ...
    def create_custom_profile(
        self, profile_name: str, base_profile: str, modifications: Dict
    ) -> Dict:
        """
        ...
        if base_profile not in self.base_profiles:
            return {
                "status": "error",
                "message": f"Base profile '{base_profile}' not found",
            }
        ...
        if "environmental_factors" in modifications:
            new_profile["environmental_factors"].update(
                modifications["environmental_factors"]
            )
    ...
    def get_profile_recommendations(
        self, plant_characteristics: Dict, environmental_conditions: Dict
    ) -> List[Dict]:
        """
        ...
        for profile_name, profile_data in self.base_profiles.items():
            ...
            humidity_key = (
                "humidity_veg" if growth_stage == "vegetative" else "humidity_flower"
            )
            ...
            if profile_name in self.profile_performance_history:
                ...
                if len(history) >= 10:
                    avg_performance = self._mean(
                        [h["irrigation_efficiency"] for h in history[-10:]]
                    )
    ...
    def import_profile_data(self, data: Dict) -> Dict:
        ...
        try:
            ...
            return {
                "status": "success",
                "imported_profiles": len(data.get("custom_profiles", {})),
            }
