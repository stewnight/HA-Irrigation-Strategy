...
try:
    ...
    from custom_components.crop_steering.llm.cost_optimizer import (
        BudgetConfig,
        CostTier,
    )
...
class LLMEnhancedCropSteering(hass.Hass):
    ...
    def initialize(self):
        ...
        try:
            ...
            if LLM_AVAILABLE and self.args.get("enable_llm", False):
                ...
                self.log(
                    "LLM components disabled or unavailable - using rule-based only"
                )
            ...
            self.log(
                f"Failed to initialize LLM-Enhanced Crop Steering: {e}", level="ERROR"
            )
            self._llm_enabled = False
    ...
    def _load_configuration(self):
        ...
        self._enable_llm_phase_transitions = self.args.get(
    ...
    def _initialize_llm_components(self):
        ...
        try:
            if not self._llm_api_key:
                self.log(
                    "LLM API key not configured - disabling LLM features",
                    level="WARNING",
                )
                self._llm_enabled = False
                return
            ...
            self.log(
                f"LLM Decision Engine initialized with {self._llm_provider.value} ({self._llm_model})"
            )
    ...
    def _setup_event_listeners(self):
        ...
        self.listen_event(
            self._handle_llm_config_update, "crop_steering_llm_config_update"
        )
    ...
    async def _process_irrigation_decision(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            # Fire response event
            self.fire_event(
                "crop_steering_irrigation_decision_response",
                {
                    "zone_id": zone_id,
    ...
    async def _get_llm_enhanced_decision(
        ...
        try:
            ...
            self.log(
                f"LLM decision for zone {zone_id}: {decision.decision} (confidence: {decision.confidence}%)"
            )
            ...
            self.log(
                f"LLM decision failed for zone {zone_id}, falling back to rules: {e}",
                level="WARNING",
            )
    ...
    async def _gather_sensor_data(self, zone_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            sensor_data["vwc_avg"] = (
                sensor_data["vwc_front"] + sensor_data["vwc_back"]
            ) / 2
            sensor_data["ec_avg"] = (
                sensor_data["ec_front"] + sensor_data["ec_back"]
            ) / 2
            ...
            self.log(
                f"Error gathering sensor data for zone {zone_id}: {e}", level="ERROR"
            )
            return {}
    ...
    async def _gather_system_config(self, zone_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            config = {
                ...
                "default_shot_size_ml": int(
                    float(self.get_state(shot_size_entity) or 100)
                ),
            ...
            self.log(
                f"Error gathering system config for zone {zone_id}: {e}", level="ERROR"
            )
            return {}
    ...
    async def _execute_irrigation_decision(self, zone_id: int, decision):
        ...
        try:
            if decision.decision == "irrigate":
                ...
                self.log(
                    f"Executed irrigation for zone {zone_id}: {decision.shot_size_ml}mL"
                )
                ...
                self.log(
                    f"Executed phase transition for zone {zone_id} to {next_phase}"
                )
    ...
    async def _process_llm_phase_transition(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            # Fire response event
            self.fire_event(
                "crop_steering_phase_transition_response",
                {
                    "zone_id": zone_id,
                    "decision": decision.decision,
                    "current_phase": current_phase,
        ...
        try:
            ...
            if hasattr(self, "_llm_engine"):
                ...
                if error_rate > 0.1:  # More than 10% error rate
                    self.log(
                        f"High LLM error rate detected: {error_rate:.2%}",
                        level="WARNING",
                    )
            ...
            if self._performance_stats["total_decisions"] > 0:
                ...
                self.log(
                    f"LLM usage rate: {llm_usage_rate:.1%}, Total cost: ${self._performance_stats['total_cost']:.4f}"
                )
