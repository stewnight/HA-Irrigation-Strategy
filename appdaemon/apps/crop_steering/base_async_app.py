...
class BaseAsyncApp(hass.Hass):
    ...
    def get_entity_value(
        self, entity_id: str, attribute: str = "state", default: Any = None
    ) -> Any:
        """
        ...
        try:
            ...
            if (
                hasattr(self, "_current_callback_is_async")
                and self._current_callback_is_async
            ):
                ...
                if inspect.iscoroutinefunction(inspect.currentframe().f_back.f_code):
                    self.log(
                        "Warning: get_entity_value called from async context. Use await self.get_state() directly.",
                        level="WARNING",
                    )
            ...
            if hasattr(result, "__await__"):
                self.log(
                    f"Task object detected for {entity_id}. Falling back to default: {default}",
                    level="DEBUG",
                )
                return default
    ...
    async def async_get_entity_value(
        self, entity_id: str, attribute: str = "state", default: Any = None
    ) -> Any:
        """
        ...
        try:
            ...
            self.log(
                f"Async error getting {attribute} for {entity_id}: {e}", level="DEBUG"
            )
            return default
    ...
    def get_float_value(self, entity_id: str, default: float = 0.0) -> float:
        ...
        try:
            ...
            if hasattr(value, "__await__"):
                self.log(
                    f"Async task detected for {entity_id}, using default: {default}",
                    level="DEBUG",
                )
                return default
            ...
            self.log(
                f"Could not convert {entity_id} value '{value}' to float, using default: {default}"
            )
            return default
    ...
    async def async_set_entity_value(
        self, entity_id: str, value: Any, **kwargs
    ) -> None:
        """
    ...
    def clear_cache(self, entity_id: Optional[str] = None) -> None:
        ...
        if entity_id:
            ...
            keys_to_delete = [
                k for k in self.entity_cache.keys() if k.startswith(f"{entity_id}:")
            ]
