...
class CostOptimizer:
    ...
    async def record_usage(
        self, response: LLMResponse, operation_type: str, zone_id: Optional[int] = None
    ) -> None:
    ...
    async def _cleanup_old_records(self) -> None:
        ...
        self._usage_records = [
            record for record in self._usage_records if record.timestamp > cutoff_date
        ]
    ...
    def get_usage_for_period(
        self, period: BudgetPeriod, reference_date: Optional[datetime] = None
    ) -> UsageStats:
        ...
        for record in period_records:
            ...
            provider_breakdown[provider] = (
                provider_breakdown.get(provider, 0) + record.cost
            )
        ...
        for record in period_records:
            ...
            operation_breakdown[op_type] = (
                operation_breakdown.get(op_type, 0) + record.cost
            )
    ...
    async def check_budget_availability(
        self, estimated_cost: float, operation_type: str = "general"
    ) -> Tuple[bool, str]:
        ...
        if daily_stats.total_cost + estimated_cost > self._config.daily_limit:
            return (
                False,
                f"Would exceed daily budget limit (${self._config.daily_limit})",
            )
        ...
        if weekly_stats.total_cost + estimated_cost > self._config.weekly_limit:
            return (
                False,
                f"Would exceed weekly budget limit (${self._config.weekly_limit})",
            )
        ...
        if monthly_stats.total_cost + estimated_cost > self._config.monthly_limit:
            return (
                False,
                f"Would exceed monthly budget limit (${self._config.monthly_limit})",
            )
    ...
    def get_cost_optimization_recommendation(
        self, operation_type: str, current_cost_estimate: float
    ) -> Tuple[bool, str, Dict]:
        ...
        daily_pct = daily_stats.total_cost / self._config.daily_limit
        weekly_stats.total_cost / self._config.weekly_limit
    ...
    async def generate_usage_report(self, days: int = 7) -> Dict:
        ...
        return {
            ...
            "current_budget_status": {
                "daily_used": round(
                    self.get_usage_for_period(BudgetPeriod.DAILY).total_cost, 4
                ),
                "daily_limit": self._config.daily_limit,
                "weekly_used": round(
                    self.get_usage_for_period(BudgetPeriod.WEEKLY).total_cost, 4
                ),
                "weekly_limit": self._config.weekly_limit,
                "monthly_used": round(
                    self.get_usage_for_period(BudgetPeriod.MONTHLY).total_cost, 4
                ),
