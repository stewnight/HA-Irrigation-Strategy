...
class ResilientLLMClient:
    ...
    def __init__(
        self, primary_client: LLMClient, fallback_client: Optional[LLMClient] = None
    ):
    ...
    async def complete(self, messages: List[Dict[str, str]], **kwargs) -> LLMResponse:
        ...
        for attempt in range(self._primary._config.max_retries):
            try:
                ...
                if self._fallback:
                    _LOGGER.warning("Primary LLM rate limited, trying fallback")
                    break
                raise
                ...
                if self._fallback:
                    _LOGGER.warning("Primary LLM failed after retries, trying fallback")
                    break
                raise
