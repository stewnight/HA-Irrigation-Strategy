...
from ..types.ml_pipeline import IrrigationPrediction, PhaseTransitionPrediction
from ..types.learning_system import LearningOutcome
...
class SafetyConstraint:
    ...
    def validate_value(
        self, value: float, context: Dict[str, Any]
        ...
        if self.min_value is not None and value < self.min_value - self.tolerance:
            return (
                False,
                f"Value {value} below minimum {self.min_value} (tolerance: {self.tolerance})",
            )

        if self.max_value is not None and value > self.max_value + self.tolerance:
            return (
                False,
                f"Value {value} above maximum {self.max_value} (tolerance: {self.tolerance})",
            )
...
class ConstraintViolation:
    ...
    violation_id: str = field(
        default_factory=lambda: f"violation_{datetime.now().isoformat()}"
    )
    timestamp: datetime = field(default_factory=datetime.now)

    # Violation details
    constraint: SafetyConstraint = field(
        default_factory=lambda: SafetyConstraint(
            "", "", "", ConstraintType.PHYSICAL_LIMITS, SafetyLevel.ADVISORY
        )
    )
...
class SafetyGuard(Protocol):
    ...
    def apply_safety_override(
        self, original_decision: Any, violations: List[ConstraintViolation]
    ) -> Any:
...
class IrrigationSafetyGuard:
    ...
    def validate_decision(
        self, decision: IrrigationPrediction, context: Dict[str, Any]
    ) -> Tuple[bool, List[ConstraintViolation]]:
        ...
        if avg_vwc > 0:
            vwc_violations = self._check_vwc_constraints(
                avg_vwc, decision, zone_id, phase
            )
        ...
        if decision.should_irrigate and decision.shot_size_ml:
            volume_violations = self._check_volume_constraints(
                decision.shot_size_ml, zone_id, phase
            )
        ...
        if last_irrigation and decision.should_irrigate:
            freq_violations = self._check_frequency_constraints(
                last_irrigation, zone_id, phase
            )
            violations.extend(freq_violations)

        # Check confidence constraints
        confidence_violations = self._check_confidence_constraints(
            decision.confidence, zone_id, phase
        )
    ...
    def _check_vwc_constraints(
        self, vwc: float, decision: IrrigationPrediction, zone_id: int, phase: str
    ...
    def _check_ec_constraints(
        self, ec: float, decision: IrrigationPrediction, zone_id: int, phase: str
    ) -> List[ConstraintViolation]:
        ...
        if ec_constraint:
            ...
            if not is_valid:
                ...
                if not decision.should_irrigate or (
                    decision.shot_size_ml and decision.shot_size_ml < 25
                ):
                    violations.append(
                        ConstraintViolation(
    ...
    def _check_volume_constraints(
        self, shot_size: int, zone_id: int, phase: str
    ) -> List[ConstraintViolation]:
        """Check irrigation volume constraints."""
        violations = []
        context = {"zone_id": zone_id, "phase": phase}
                ...
                violations.append(
                    ConstraintViolation(
                        ...
                        immediate_actions_taken=[
                            "Increase shot size or skip irrigation"
                        ],
                    )
                )
    ...
    def _check_frequency_constraints(
        self, last_irrigation: datetime, zone_id: int, phase: str
    ) -> List[ConstraintViolation]:
    ...
    def _check_confidence_constraints(
        self, confidence: float, zone_id: int, phase: str
    ) -> List[ConstraintViolation]:
    ...
    def _check_daily_limits(
        self, projected_usage: float, zone_id: int, phase: str
    ) -> List[ConstraintViolation]:
    ...
    def get_safety_actions(
        ...
        for violation in violations:
            if violation.constraint.safety_level == SafetyLevel.CRITICAL:
                ...
                actions.append(
                    (
                        SafetyAction.TRIGGER_EMERGENCY_STOP,
                        {
                ...
                if "min_ai_confidence" in violation.constraint.constraint_id:
                    actions.append(
                        (
                            SafetyAction.FALLBACK_TO_RULES,
                        ...
                        (
                            SafetyAction.BLOCK_DECISION,
                            {
                                ...
                                "retry_after_seconds": int(
                                    violation.constraint.min_value
                                    - violation.violating_value
                                ),
                            },
                        )
                    )
    ...
    def apply_safety_override(
        ...
        for action, params in actions:
            if action == SafetyAction.BLOCK_DECISION:
                ...
                safe_decision.reasoning += (
                    f" - BLOCKED: {params.get('reason', 'Safety constraint')}"
                )
        ...
        if len(actions) > 0:
            critical_actions = [
                ...
                for a, p in actions
                if a
                in [SafetyAction.TRIGGER_EMERGENCY_STOP, SafetyAction.FORCE_SAFE_VALUE]
            ]
            if critical_actions:
                safe_decision.urgency_score = min(
                    1.0, safe_decision.urgency_score + 0.3
                )
...
class LearningSafetyGuard:
    ...
    def validate_learning_outcome(
        self, outcome: LearningOutcome, context: Dict[str, Any]
    ) -> Tuple[bool, List[ConstraintViolation]]:
        ...
        for param, (old_val, new_val) in outcome.parameters_optimized.items():
            if old_val != 0:  # Avoid division by zero
                ...
                if change_ratio > 0.2:  # 20% change limit
                    violations.append(
                        ConstraintViolation(
                            constraint=self._get_learning_constraint(
    ...
    def _get_learning_constraint(
        self, constraint_id: str
    ) -> Optional[SafetyConstraint]:
